...
from components.menu.edit.common_utils import handle_tab_button_click
...
class TypeChanges:
    ...
    def __init__(self):
        ...
        self.conversion_options = {
            "str": [{"label": "기본 변환", "value": "default"}, {"label": "소문자로 변환", "value": "lowercase"}, {"label": "대문자로 변환", "value": "uppercase"}],
            "int": [{"label": "기본 변환", "value": "default"}, {"label": "반올림", "value": "round"}, {"label": "내림", "value": "floor"}, {"label": "올림", "value": "ceil"}],
            "float": [{"label": "기본 변환", "value": "default"}, {"label": "소수점 2자리", "value": "2decimal"}, {"label": "소수점 4자리", "value": "4decimal"}],
            "bool": [{"label": "기본 변환", "value": "default"}, {"label": "문자열 (true/false, yes/no 등) 인식", "value": "string_recognize"}],
        }

    def button_layout(self):
        return dbpc.Button("Type Changes", id="type-changes-btn", icon="data-lineage", minimal=True, outlined=True)
            ...
            children=[
                ...
                dmc.Paper(
                    ...
                    children=[
                        ...
                        dmc.MultiSelect(id="type-changes-column-select", label="컬럼 선택", description="타입을 변경할 컬럼을 선택하세요 (복수 선택 가능)", placeholder="컬럼 선택...", required=True, searchable=True, clearable=True, data=[], size="md", leftSection=dbpc.Icon(icon="properties")),
                        dmc.Space(h=10),
                ...
                dmc.Paper(
                    ...
                    children=[
                        ...
                        dmc.Select(
                            ...
                            data=[{"label": "📝 문자열 (String)", "value": "str"}, {"label": "🔢 정수 (Integer)", "value": "int"}, {"label": "📊 실수 (Float)", "value": "float"}, {"label": "✓✗ 불리언 (Boolean)", "value": "bool"}],
                            size="md",
                        ),
                        dmc.Space(h=15),
                        # 변환 옵션 선택 (타입에 따라 동적으로 변경)
                        dmc.Select(id="type-changes-conversion-option", label="변환 옵션", description="데이터 변환 방식을 선택하세요", placeholder="변환 옵션 선택...", data=[], size="md", disabled=True),
                        # 현재 선택한 변환 옵션에 대한 설명
                        html.Div(id="type-changes-option-description", style={"marginTop": "10px"}),
                        dmc.Space(h=15),
                        # 변환 실패 처리 옵션
                        dmc.RadioGroup(id="type-changes-fail-option", label="변환 실패 시 처리 방법", description="변환할 수 없는 값이 있을 경우 어떻게 처리할지 선택하세요", value="null", size="sm", children=[dmc.Radio(value="null", label="Null 값으로 대체"), dmc.Radio(value="default", label="기본값으로 대체"), dmc.Radio(value="error", label="오류 발생 (변환 취소)")]),
                        # 기본값 입력 필드 (fail_option이 'default'일 때만 표시)
                        html.Div(id="type-changes-default-value-container", style={"display": "none"}, children=[dmc.TextInput(id="type-changes-default-value", label="기본값", description="변환 실패 시 적용할 기본값을 입력하세요", placeholder="기본값 입력...", size="sm")]),
                    ],
                ),
                # 미리보기 영역
                dmc.Space(h=10),
                dmc.Text("3. 변환 미리보기", fw=700, mb=5),
                dmc.Paper(id="type-changes-preview-container", withBorder=True, p="sm", style={"maxHeight": "300px", "overflow": "auto"}, children=[dmc.Text("미리보기: 컬럼과 타입을 선택하면 변환 예시가 표시됩니다.", size="sm", c="dimmed")]),
                dmc.Space(h=20),
                # 변환 실행 버튼
                dmc.Group(
                    [
    ...
    def register_callbacks(self, app):
        ...
        @app.callback(Output("flex-layout", "model", allow_duplicate=True), Output("toaster", "toasts", allow_duplicate=True), Input("type-changes-btn", "n_clicks"), State("flex-layout", "model"), prevent_initial_call=True)
        def handle_type_change_button_click(n_clicks, current_model):
            """Add Row 버튼 클릭 시 우측 패널에 탭 추가"""
            return handle_tab_button_click(n_clicks, current_model, "type-changes-tab", "Type Changes")

        @app.callback(Output("type-changes-column-select", "data"), Input("type-changes-btn", "n_clicks"), State("aggrid-table", "columnDefs"), prevent_initial_call=True)
        def update_column_list(n_clicks, columnDefs):
            ...
            for col in columnDefs:
                ...
                if col_field not in SYSTEM_COLUMNS:
                    ...
                    column_data.append({"label": f"{type_icon} {col_field} ({col_type})", "value": col_field})

            return column_data

        @app.callback(Output("type-changes-info-container", "children"), Input("type-changes-column-select", "value"), prevent_initial_call=True)
            ...
            for col in selected_columns:
                try:
                    ...
                    info_component = dmc.Paper(
                        ...
                        mb="xs",
                        children=[
                            dmc.Group([dmc.Group([dmc.Text(type_icon, size="xl", mr=0), dmc.Text(f"컬럼: {col}", w=500)], gap="xs"), dmc.Badge(readable_type, color=type_color, size="lg")], justify="apart"),
                            dmc.Divider(my="xs"),
                            # 데이터 요약 정보
                            dmc.SimpleGrid(
                                cols=2,
                    ...
                    info_components.append(dmc.Alert(f"컬럼 '{col}' 정보를 가져올 수 없습니다: {str(e)}", color="red", variant="light", mb="xs"))

            return info_components

        @app.callback(Output("type-changes-conversion-option", "data"), Output("type-changes-conversion-option", "disabled"), Input("type-changes-target-type", "value"), prevent_initial_call=True)
        ...
        @app.callback(Output("type-changes-default-value-container", "style"), Input("type-changes-fail-option", "value"), prevent_initial_call=True)
        ...
        @app.callback(Output("type-changes-option-description", "children"), [Input("type-changes-target-type", "value"), Input("type-changes-conversion-option", "value")], prevent_initial_call=True)
        def update_option_description(target_type, conversion_option):
            ...
            descriptions = {"str": {"lowercase": "모든 문자를 소문자로 변환합니다. (예: 'ABC' → 'abc')", "uppercase": "모든 문자를 대문자로 변환합니다. (예: 'abc' → 'ABC')", "titlecase": "각 단어의 첫 글자를 대문자로 변환합니다. (예: 'hello world' → 'Hello World')"}, "int": {"round": "소수점 값을 반올림하여 정수로 변환합니다. (예: 3.7 → 4)", "floor": "소수점 값을 내림하여 정수로 변환합니다. (예: 3.7 → 3)", "ceil": "소수점 값을 올림하여 정수로 변환합니다. (예: 3.2 → 4)"}, "float": {"2decimal": "소수점 2자리까지 유지하고 반올림합니다. (예: 3.14159 → 3.14)", "4decimal": "소수점 4자리까지 유지하고 반올림합니다. (예: 3.14159 → 3.1416)", "scientific": "과학적 표기법으로 변환합니다. (예: 1000000 → 1.00e+6)"}, "bool": {"string_recognize": "문자열을 불리언으로 해석합니다. 'true', 'yes', '1', 'y'는 True로, 'false', 'no', '0', 'n'은 False로 변환합니다."}}
        ...
        @app.callback(Output("type-changes-preview-container", "children"), [Input("type-changes-column-select", "value"), Input("type-changes-target-type", "value"), Input("type-changes-conversion-option", "value"), Input("type-changes-fail-option", "value"), Input("type-changes-default-value", "value")], prevent_initial_call=True)
        def update_preview(selected_columns, target_type, conversion_option, fail_option, default_value):
            ...
            # 변환 설정 요약
            target_type_name = {"str": "문자열 (String)", "int": "정수 (Integer)", "float": "실수 (Float)", "bool": "불리언 (Boolean)"}.get(target_type, target_type)

            conversion_option_name = "기본 변환"
            if conversion_option and conversion_option != "default":
                conversion_option_names = {"lowercase": "소문자 변환", "uppercase": "대문자 변환", "round": "반올림", "floor": "내림", "ceil": "올림", "2decimal": "소수점 2자리", "4decimal": "소수점 4자리", "scientific": "과학적 표기법", "string_recognize": "문자열 인식"}
                conversion_option_name = conversion_option_names.get(conversion_option, conversion_option)

            fail_option_name = {"null": "Null 값으로 대체", "default": f"기본값({default_value})으로 대체" if default_value else "기본값으로 대체", "error": "오류 발생 (변환 취소)"}.get(fail_option, fail_option)
            ...
            # 테이블 헤더 준비
            thead = dmc.TableThead(dmc.TableTr([dmc.TableTh("컬럼"), dmc.TableTh("원본 값"), dmc.TableTh("변환 값"), dmc.TableTh("결과")]))
            ...
            for col in selected_columns:
                try:
                    ...
                    if not sample_values:
                        ...
                        table_rows.append(dmc.TableTr([dmc.TableTd(col), dmc.TableTd("데이터 없음", colSpan=3, style={"textAlign": "center", "color": "gray"})]))
                        continue
                    ...
                    try:
                        ...
                        # 행 추가
                        table_rows.append(dmc.TableTr([dmc.TableTd(col), dmc.TableTd(orig_display), dmc.TableTd(conv_display), dmc.TableTd(dmc.Badge("성공", color="green", variant="light", size="sm"))]))
                    except Exception as e:
                        # 변환 실패
                        error_msg = str(e)
                        ...
                        # 행 추가
                        table_rows.append(dmc.TableTr([dmc.TableTd(col), dmc.TableTd(f"{value} ({type(value).__name__})"), dmc.TableTd(conv_display), dmc.TableTd(result_badge)]))

                except Exception as e:
                    logger.error(f"미리보기 생성 오류 (컬럼 {col}): {str(e)}")
                    table_rows.append(dmc.TableTr([dmc.TableTd(col), dmc.TableTd(f"오류: {str(e)}", colSpan=3, style={"color": "red"})]))

            # 테이블 본문
            tbody = dmc.TableTbody(table_rows)

            # 최종 테이블
            preview_table = dmc.Table([thead, tbody], striped=True, highlightOnHover=True, withTableBorder=True, withColumnBorders=True)

            preview_content.append(preview_table)

            # 주의사항 추가
            if target_type == "int":
                preview_content.append(dmc.Alert("정수형 변환 시 소수점이 있는 값은 변환 옵션에 따라 반올림/내림/올림됩니다.", color="yellow", variant="light", mt="md"))
            elif target_type == "bool":
                preview_content.append(dmc.Alert("불리언 변환에서 'true', 'yes', '1', 'y', 't', 'on'은 True로, 'false', 'no', '0', 'n', 'f', 'off'는 False로 변환됩니다.", color="yellow", variant="light", mt="md"))

            return preview_content

        @app.callback(Output("toaster", "toasts", allow_duplicate=True), Output("aggrid-table", "columnDefs", allow_duplicate=True), Output("type-changes-apply-btn", "loading"), Output("type-changes-column-select", "value"), [Input("type-changes-apply-btn", "n_clicks")], [State("type-changes-column-select", "value"), State("type-changes-target-type", "value"), State("type-changes-conversion-option", "value"), State("type-changes-fail-option", "value"), State("type-changes-default-value", "value")], prevent_initial_call=True)  # 컬럼 선택 초기화를 위한 출력 추가
        def apply_type_changes(n_clicks, selected_columns, target_type, conversion_option, fail_option, default_value):
            """타입 변환 적용 - 최적화"""
            ...
            try:
                ...
                if not target_polars_type:
                    return ([dbpc.Toast(message=f"지원하지 않는 데이터 타입입니다: {target_type}", intent="danger", icon="error")], no_update, False, no_update)
                    ...
                    try:
                        ...
                        if conversion_option and conversion_option != "default":
                            ...
                            if target_polars_type == pl.Boolean and conversion_option == "string_recognize":
                                # 불리언 문자열 인식 로직
                                df = df.with_columns(pl.when(pl.col(col).cast(pl.Utf8).str.to_lowercase().is_in(["true", "1", "yes", "y", "on", "t"])).then(True).when(pl.col(col).cast(pl.Utf8).str.to_lowercase().is_in(["false", "0", "no", "n", "off", "f"])).then(False).otherwise(None).alias(col))
                            else:
                                # 사용자 정의 변환 함수 적용
                                df = df.with_columns(pl.col(col).map_elements(conversion_func, return_dtype=target_polars_type).alias(col))
                        else:
                            # 기본 타입 변환
                            if fail_option == "null":
                                # 변환 실패 시 null 값으로 대체
                                ...
                                # 변환 실패 시 기본값으로 대체
                                try:
                                    df = df.with_columns(pl.coalesce(pl.col(col).cast(target_polars_type, strict=False), pl.lit(fill_value)).alias(col))
                                except:
                ...
                if failed_columns:
                    ...
                    if successful_columns:
                        ...
                        return ([dbpc.Toast(message=f"{len(successful_columns)}개 컬럼 변환 성공, {len(failed_columns)}개 실패\n{error_messages}", intent="warning", icon="warning-sign", timeout=4000)], updated_columnDefs, False, [])  # 컬럼 선택 초기화
                    else:
                        # 모든 컬럼 변환 실패
                        return ([dbpc.Toast(message=f"모든 컬럼 변환 실패:\n{error_messages}", intent="danger", icon="error", timeout=4000)], no_update, False, no_update)  # 컬럼 선택 유지
                ...
                return ([dbpc.Toast(message=f"{len(successful_columns)}개 컬럼의 타입이 '{target_type_name}'으로 변환되었습니다.", intent="success", icon="endorsed", timeout=3000)], updated_columnDefs, False, [])  # 컬럼 선택 초기화

            except Exception as e:
                # 전체 처리 오류
                logger.error(f"타입 변환 처리 오류: {str(e)}")
                return ([dbpc.Toast(message=f"타입 변환 처리 오류: {str(e)}", intent="danger", icon="error")], no_update, False, no_update)  # 컬럼 선택 유지
