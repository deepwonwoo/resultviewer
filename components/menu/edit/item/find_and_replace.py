...
class FindAndReplace:
    def __init__(self):
        ...
        self.case_sensitive_options = [
            ["insensitive", "대소문자 구분 안함"],
            ["sensitive", "대소문자 구분"]
        ]
        
        # 정규식 예제
        self.regex_examples = [
            {"value": r"\d+", "label": "숫자만 찾기", "description": "모든 숫자를 찾습니다 (예: 123, 45)"},
            {"value": r"[a-zA-Z]+", "label": "알파벳만 찾기", "description": "알파벳 문자열을 찾습니다"},
            {"value": r"^\w+", "label": "시작 문자열", "description": "줄의 시작 문자열을 찾습니다"},
            {"value": r"\w+$", "label": "끝 문자열", "description": "줄의 끝 문자열을 찾습니다"},
            {"value": r"\s+", "label": "공백 문자", "description": "하나 이상의 공백을 찾습니다"},
            {"value": r"[A-Z]{2,}", "label": "대문자 연속", "description": "2개 이상 연속된 대문자를 찾습니다"}
        ]
    ...
    def tab_layout(self):
        return dmc.Paper(
            children=[
                dmc.Group(
                    [
                        dbpc.Icon(icon="search-template", size=24),
                        dmc.Title("Find and Replace", order=3)
                    ],
                    mb="md"
                ),
                ...
                dmc.Card(
                    children=[
                                    ...
                                    dmc.RadioGroup(
                                        ...
                                        children=dmc.Stack([
                                            dmc.Radio(
                                                ...
                                                value=k,
                                                size="sm"
                                            ) for k, l in self.case_sensitive_options
                                        ], gap="xs"),
                                    )
                                ]
                            )
                        ...
                        html.Div(
                            ...
                            style={"display": "none"},
                            children=[
                                dmc.Divider(label="정규식 도움말", labelPosition="center", my="md"),
                                dmc.SimpleGrid(
                                    cols=2,
                                    spacing="md",
                                    children=[
                                        dmc.Paper(
                                            ...
                                            children=[
                                                dmc.Group([
                                                    dmc.Badge(ex["label"], color="blue", variant="light"),
                                                    dmc.Code(ex["value"], color="blue")
                                                ], mb="xs"),
                                                dmc.Text(ex["description"], size="sm", c="dimmed")
                                            ]
                                        ) for ex in self.regex_examples
                                    ]
                                ),
                                dmc.Center(
                                    dmc.Button(
                                        "정규식 예제 복사",
                                        id="regex-example-select",
                                        variant="light",
                                        size="xs",
                                        mt="sm"
                                    )
                        ...
                        dmc.Checkbox(
                            ...
                            label="필터링된 데이터에만 적용",
                            description="현재 필터링된 데이터에만 검색 및 치환을 적용합니다",
                            size="sm",
                            checked=False
                        )
                    ],
                ...
                dmc.Card(
                    ...
                    children=[
                        dmc.ScrollArea(
                            ...
                            h=200,
                            children=[dmc.Text("검색 결과가 여기에 표시됩니다.", size="sm", c="dimmed")]
                        )
                    ]
                ),
    ...
    def register_callbacks(self, app):
        ...
        # 정규식 예제 선택 모달
        @app.callback(
            Output("find-replace-search-input", "value"),
            Input("regex-example-select", "n_clicks"),
        ...
        @app.callback(
            Output("find-replace-preview-card", "style"),
        ...
        @app.callback(
            ...
            [
                ...
                State("find-replace-case-sensitive", "value"),
            ...
            try:
                ...
                for col in selected_columns:
                    ...
                    if col_matches > 0:
                        matched_rows.append({
                            "column": col,
                            "count": col_matches,
                            "sample": matches.head(5)[col].to_list()
                        })
                ...
                for row in matched_rows:
                    preview_content.append(
                        dmc.Card([
                            dmc.Group([
                                dmc.Text(f"컬럼: {row['column']}", w=500),
                                dmc.Badge(f"{row['count']}개", color="blue", variant="light")
                            ], mb="xs"),
                            dmc.ScrollArea(
                                dmc.List([
                                    dmc.ListItem(
                                        dmc.Group([
                                            ...
                                            dbpc.Icon(icon="arrow-right", color="gray"),
                                            dmc.Badge(
                                                self._apply_replacement(item, search_value, replace_value, mode, case_sensitive),
                                                color="green",
                                                variant="light",
                                                style={"maxWidth": "200px", "overflow": "hidden", "textOverflow": "ellipsis"}
                                            )
                        ...
                        ], withBorder=True, p="sm", mb="xs", shadow="sm")
                    )
        ...
        @app.callback(
            [
                ...
                Output("find-replace-preview-container", "children", allow_duplicate=True),
                Output("find-replace-preview-card", "style", allow_duplicate=True),
                Output("find-replace-apply-btn", "disabled", allow_duplicate=True)
            ],
        ...
        def apply_find_replace(n_clicks, selected_columns, search_value, replace_value, mode, case_sensitive, filtered_only):
            ...
            try:
                ...
                for col in selected_columns:
                    ...
                    # 치환된 행 개수 계산
                    if filtered_only:
                        matches = df.filter(pl.col("uniqid").is_in(filtered_ids)).filter(
                ...
                return [
                    dbpc.Toast(
                        message=f"{total_replacements}개의 항목이 성공적으로 치환되었습니다.",
                        intent="success",
                        icon="endorsed",
                        timeout=3000
                    )
                ], updated_columnDefs, "", "", [dmc.Text("검색 결과가 여기에 표시됩니다.", size="sm", c="dimmed")], {"display": "none"}, True
                
            except Exception as e:
                logger.error(f"치환 처리 오류: {str(e)}")
                return [
                    dbpc.Toast(
                        message=f"치환 처리 오류: {str(e)}",
                        intent="danger",
                        icon="error"
                    )
