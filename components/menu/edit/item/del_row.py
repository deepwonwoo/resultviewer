...
class DelRow:
                            ...
                            dmc.AccordionItem(
                                [
                                    ...
                                    dmc.AccordionPanel([
                                        ...
                                        dmc.Text("2. '필터 정보 갱신' 버튼을 클릭하여 현재 필터 상태를 확인합니다."),
    ...
    def register_callbacks(self, app):
        ...
        app.clientside_callback(
            ...
            """,
            [Output("delete-row-filter-model", "data"),
             Output("delete-row-filter-info", "data")],
        ...
        @app.callback(
            ...
            Input("delete-row-filter-info", "data")
        )
        ...
        @app.callback(
            ...
            [
                State("delete-row-filter-model", "data"),
                State("delete-row-filter-info", "data")
            ],
            prevent_initial_call=True
        )
        def handle_delete_row_submission(n_clicks, filter_model, filter_info):
            ...
            try:
                ...
                if not has_filter or not filter_model or len(filter_model) == 0:
                    return ([dbpc.Toast(
                        message="필터가 적용되지 않았습니다. 특정 행만 필터링한 후 시도해주세요.",
                        intent="warning",
                        icon="warning-sign"
                    )], no_update, False)
                ...
                if filtered_count >= original_row_count:
                    return ([dbpc.Toast(
                        message="필터 조건에 모든 행이 포함되어 있어 삭제할 수 없습니다.",
                        intent="warning",
                        icon="warning-sign"
                    )], no_update, False)
