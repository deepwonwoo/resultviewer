    ...
    try:
        ...
        if groupBy:
            if groupKeys:
                ...
                for i in range(len(groupKeys)):
                    if groupKeys[i] is None:
                        ...
                        if i + 1 < len(
                            ...
                            total_count = len(
                                group_counts_next["count"]
                            )  # 전체 개수 (이 그룹에 속하는 모든 하위 그룹의 수)
                ...
                group_counts = df.group_by(groupBy[0]).agg(
                    pl.len().alias("childCount")
                )  # no group_keys
                if agg:
                    agg_expressions = [
                    ...
                    df_agg = df.group_by(groupBy[0], maintain_order=True).agg(
                        [pl.first("*")]
                    )
                df = df_agg.join(
                    group_counts, on=groupBy[0], how="left"
                )  # 그룹별 자식 수와 집계 결과를 합침
                if "waiver" in df.columns and "waiver" not in groupBy:
                    df = df.drop("waiver")
                    df = df.with_columns(pl.lit("").alias("waiver"))
