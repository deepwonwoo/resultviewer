...
from utils.db_management import ROW_COUNTER, get_dataframe, set_cache, get_cache
    ...
    def process_conditions(df, conditions, operator):
        if operator == "AND":
            for condition in conditions:
                df = process_conditions(df, condition["conditions"], condition["type"]) if "conditions" in condition else apply_filter_condition(df, condition)
        elif operator == "OR":
            filtered_dfs = [process_conditions(df.clone(), condition["conditions"], condition["type"]) if "conditions" in condition else apply_filter_condition(df.clone(), condition) for condition in conditions]
            df = pl.concat(filtered_dfs).unique(maintain_order=True)
        return df

    try:
        ROW_COUNTER["filtered"] = 0
        df = apply_filter_condition(df, filterModel) if "colId" in filterModel else process_conditions(df, filterModel["conditions"], filterModel.get("type", "AND"))
...
def apply_group(df, request):
    agg_function_mapping = {
        "avg": pl.mean, "count": pl.count, "first": pl.first,
        "last": pl.last, "min": pl.min, "max": pl.max, "sum": pl.sum,
    }
    ...
    try:
        ...
        if get_cache("hide_waiver") and "waiver" in df.columns:
        ...
        if groupKeys:
            for i, key in enumerate(groupKeys):
                if key is None:
                    return df.clear()
                df = df.filter(pl.col(groupBy[i]) == key)

            group_counts = df.group_by(groupBy[:len(groupKeys) + 1]).agg(pl.len().alias("childCount"))
            ROW_COUNTER["groupby"] = f"{len(group_counts)} ({', '.join([f'{k}: {v}' for k, v in zip(groupKeys, group_counts['childCount'])])})"

            if len(groupKeys) != len(groupBy):
                if agg:
                    df_agg = df.group_by(groupBy[:len(groupKeys) + 1]).agg([agg_function_mapping[func](col) for col, func in agg.items()])
            ...
            if agg:
                df_agg = df.group_by(groupBy[0]).agg([agg_function_mapping[func](col) for col, func in agg.items()])
            else:
                df_agg = df.group_by(groupBy[0]).agg([pl.first("*")])

            df = df_agg.join(group_counts, on=groupBy[0], how="left").sort(groupBy[0])
            if "waiver" in df.columns and "waiver" not in groupBy:
                df = df.drop("waiver").with_columns(pl.lit("").alias("waiver"))
            df = df.with_columns(pl.lit(True).alias("group"))
...
def apply_sort(df, request):
    ...
    try:
        ...
        if groupBy and "childCount" in df.columns:
            ...
            df = df.sort([s for s in sorting if s in df.columns], descending=[not a for a in asc if sorting[asc.index(a)] in df.columns])
